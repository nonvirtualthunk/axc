...
TEST_CASE("TextLayout","[gui][font][text]") {
    ...
    SECTION("All can fit on one line") {
        ...
        int fontSize = 20;
        Sequence<GlyphLayout> layout = TextLayout::layoutText(Seq({text}),font,fontSize,10000);
        ...
        bool sameLine = layout.forall([&](const GlyphLayout& g) {
            return std::abs(g.location.y - layout.head().location.y) < fontSize*0.75;
        });
    ...
    SECTION("All but last word can fit on line") {
        String text = "This is a test";
        int fontSize = 20;
        Sequence<GlyphLayout> layout = TextLayout::layoutText(Seq({text}),font,fontSize,90);
